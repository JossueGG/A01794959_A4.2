...
Description:
    ...
    The results are printed to the screen and saved to a file named "WordCountResults.txt".
    All computations are performed using basic algorithms, not relying on advanced
...
def read_words(file_name):
    ...
    If a line is empty or an error occurs during reading, an error message is printed
    ...
    try:
        with open(file_name, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if not line:
                    continue
                # Basic splitting: traverse each character and build words.
                current_word = ""
                for char in line:
                    if char.isspace():
                        if current_word != "":
...
def count_words(words):
    ...
    The counting is done in a case-insensitive way.
    ...
    freq = {}
    for word in words:
        # Convert to lower case manually (for ASCII letters)
        w = ""
        for char in word:
            if 'A' <= char <= 'Z':
                w += chr(ord(char) + 32)
...
def format_results(freq, elapsed_time):
    ...
    # Sort words alphabetically using a basic bubble sort algorithm.
    keys = list(freq.keys())
    n = len(keys)
    for i in range(n):
